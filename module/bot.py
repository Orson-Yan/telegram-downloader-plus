...
async def send_help_str(client: pyrogram.Client, chat_id):
    ...
    msg = (
            ...
            f"/help - {_t('Show this help message')}\n"
            f"/start - {_t('Show this help message')}\n"
            f"/get_info <link> - {_t('Get group/channel info from a message link')}\n"
            f"/download <link> <start_id> <end_id> [filter] - {_t('Batch download messages')}\n"
            f"/forward <src_link> <dst_link> <start_id> <end_id> [filter] - {_t('Forward messages between chats')}\n"
            f"/forward_to_comments <src_link> <dst_link> <start_id> <end_id> - {_t('Forward media to comment section')}\n"
            f"/listen_forward <src_link> <dst_link> [filter] - {_t('Auto-forward new messages in real-time')}\n"
            f"/stop - {_t('Stop download / forward / listen_forward (interactive)')}\n"
            f"/set_language en|ru|zh|ua - {_t('Set bot language')}\n"
            f"/add_filter <filter> - {_t('Set download filter')}\n\n"
            f"{_t('**Quick actions (no command needed):**')}\n"
            f"• {_t('Send a Telegram message link')} - {_t('Download a single message')}\n"
            f"• {_t('Forward a media message to the bot')} - {_t('Download or re-upload that media')}\n\n"
            f"{_t('**Notes**')}\n"
            f"• `[option]` {_t('means optional, not required')}\n"
            f"• {_t('start_id=1 means from the beginning, end_id=0 means to the end')}\n"
        )
